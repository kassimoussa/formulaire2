...
class GestionClient2 extends Component
{
    use WithPagination;
    ...
    {
        /* $search = $this->search;
        $this->clients = Client::where('user_id', $this->user_id)->Where(function ($query) use ($search) {
    ...
    {
        $this->reset([
           'search', 'client_id', 'nom', 'numero', 'date_naissance', 'lieu_naissance', 'domicile', 'profession', 'type_piece', 'id_piece', 'date_emission', 'date_expiration',
    ...
    public function render()
    {
        ...
        $search = $this->search;
        $clients = Client::where('user_id', $this->user_id)->Where(function ($query) use ($search) {
            $query->where('numero', 'Like', '%' . $search . '%')
                ->orWhere('nom', 'Like', '%' . $search . '%')
                ->orWhere('type_piece', 'Like', '%' . $search . '%');
        })->orderBy("created_at", "asc")
            ->paginate(20);
        return view('livewire.gestion-client2', ['clients' => $clients]);
    }
}
